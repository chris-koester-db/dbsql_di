...
def generate_data(datagen_src_path, tpcdi_directory, catalog, scale_factor, UC_enabled=True, lighthouse=False):
  ...
  print(f"Generating data for scale factor={scale_factor} in directory {blob_out_path}")
  copy_directory(datagen_src_path, driver_tmp_path, overwrite=True)
...
def DIGen(digen_path, scale_factor, output_path):
  cmd = f"java -jar {digen_path}DIGen.jar -sf {str(scale_factor)} -o {output_path}"
...
# Hard code scale_factor to 10 since this is for demonstrating
# DBSQL workflows, not benchmarking different scale factors
scale_factor = 10
datagen_src_path = f'{Path.cwd()}/datagen'
tpcdi_directory = f'/Volumes/{catalog}/tpcdi_raw_data/tpcdi_volume/'

generate_data(datagen_src_path, tpcdi_directory, catalog, scale_factor)
